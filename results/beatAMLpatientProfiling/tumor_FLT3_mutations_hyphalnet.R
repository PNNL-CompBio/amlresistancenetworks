...
hyp_type = AML %>% filter(AML$hyp1 == "mutations" & AML$hyp2 == "mutations")
community = hyp_type %>% filter(hyp_type$net2_type == "community")

community$category = rep('FLT3 Mutation',nrow(community))
community$category[grep('NO',community$FLT3)]<-'No FLT3 Mutation'

#perform ttests for communities, based on grouping whether samples have mutation or not
ttest_comm = function(x){
  data_FLT3 = community %>% filter(net2 == x)
  normal_FLT3 = data_FLT3 %>% filter(str_detect(FLT3,'NO'))
  mutation_FLT3 = data_FLT3 %>% filter(str_detect(FLT3,'YES'))
  model = t.test(mutation_FLT3$distance, normal_FLT3$distance, paired = FALSE)
  return (model)
}
